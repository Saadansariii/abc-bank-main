...
export class ManualFourRowComponent implements OnInit, AfterViewInit {
  ...
  apiAllUrl =
    'http://167.172.220.75:8084/CashflowForecastingApplication/api/corporate/all?page=0&size=3';
  apiSearchByCodeUrl =
    'http://167.172.220.75:8084/CashflowForecastingApplication/api/corporate/corporate-code';
  apiSearchByNameUrl =
    'http://167.172.220.75:8084/CashflowForecastingApplication/api/corporate/corporate-name';
  accountOptions = {
    internalAccount: [
  ...
  onAccountTypeChange() {
    if (this.formData.corporateCode && this.formData.accountType) {
      const url = `http://167.172.220.75:8084/CashflowForecastingApplication/api/accounts/by-type?accountType=${this.formData.accountType}&corporateId=${this.corporateCodeId}`;
      this.http
        .get(url)
        .pipe(
          catchError((error) => {
      ...
      alert('Please add Corporate Code and select an Account Type first');
    }
  }
      ...
      this.http
        ...
        .pipe(
          catchError((error) => {
            ...
            alert('Error occurred while saving the forecast.');
            return throwError(error);
          })
        )
        .subscribe((response) => {
